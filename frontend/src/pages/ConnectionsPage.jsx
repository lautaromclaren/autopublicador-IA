// src/pages/ConnectionsPage.jsx - VERSIÓN FINAL Y COMPLETA CON HEADER
...
function ConnectionsPage() {
  ...
  const handleGroupSelect = (groupId) => { setSelectedGroups(prev => ({ ...prev, [groupId]: !prev[groupId] })); };
  ...
  const handleDeleteSet = async (setId) => {
    ...
    try {
      ...
      await fetchGroupSets();
    } catch (err) { setError('No se pudo borrar el conjunto.'); }
    finally { setIsLoading(false); }
  };
  ...
  const handleMockConnect = async () => {
    setIsLoading(true);
    try {
      await apiClient.get('/auth/facebook/callback/mock');
      refetchUser();
    } catch (error) { setError('Error al simular la conexión.'); }
    finally { setIsLoading(false); }
  };
      ...
      <main className="main-container">
        ...
        {!isFacebookConnected && (
          <div style={{ marginTop: '2rem', border: '1px solid #ccc', padding: '2rem', borderRadius: '8px' }}>
            ...
            <button onClick={handleMockConnect} disabled={isLoading}>{isLoading ? 'Procesando...' : 'Simular Conexión'}</button>
          </div>
        )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem', marginTop: '2rem' }}>
            <div>
              ...
              <div style={{ maxHeight: '400px', overflowY: 'auto', border: '1px solid #eee', padding: '1rem', borderRadius: '8px', background: '#fff' }}>
                {isLoading && groups.length === 0 ? <p>Cargando...</p> : groups.map(group => (
            ...
            <div>
              ...
              <div style={{ border: '1px solid #eee', padding: '1rem', minHeight: '265px', marginBottom: '1rem', borderRadius: '8px', background: '#fff' }}>
                {isLoading && groupSets.length === 0 ? <p>Cargando...</p> : null}
                {groupSets.map(set => (
                  <div key={set._id} style={{ padding: '0.8rem', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span><strong>{set.name}</strong> ({set.groups.length} grupos)</span>
                    <button onClick={() => handleDeleteSet(set._id)} disabled={isLoading} style={{ background: '#dc3545', color: 'white', border: 'none', borderRadius: '4px', padding: '0.3rem 0.6rem', cursor: 'pointer' }}>Borrar</button>
                  </div>
                ))}
                {groupSets.length === 0 && !isLoading && <p style={{ textAlign: 'center', color: '#666' }}>Aún no has creado ninguna carpetita.</p>}
              </div>
              <form onSubmit={handleCreateSet}>
                <input type="text" value={newSetName} onChange={(e) => setNewSetName(e.target.value)} placeholder="Nombre de la nueva carpetita" required style={{ width: '100%', padding: '0.8rem', marginBottom: '0.5rem', fontSize: '1rem' }} />
